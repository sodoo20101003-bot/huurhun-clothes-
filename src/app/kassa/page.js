...
export default function KassaPage() {
  ...
  // Холимог төлбөр: { cash: 0, card: 0, pocket: 0, storepay: 0, dans: 0 }
  ...
  // Сонгосон ангилалд харьяалагдах брэндүүд
  ...
  // === Сагсны нийт тооцоо (pair_price автомат) ===
  ...
  function addToCart(product, variant) {
    ...
    if (existing >= 0) {
      setCart((c) => {
        const next = [...c];
        if (next[existing].qty + 1 > variant.stock) {
  ...
  function setPayment(method, value) {
    setPayments((p) => ({ ...p, [method]: Number(value) || 0 }));
  }

  function autoFillRemaining(method) {
    // Үлдсэн дүнг автоматаар тэр төлбөрт хийнэ
    const others = Object.entries(payments)
      .filter(([k]) => k !== method)
  ...
  async function complete() {
    ...
    if (Math.abs(remaining) > 1) {
      return alert(`Төлбөрийн дүн таарахгүй байна!\n\nҮлдсэн: ${formatPrice(remaining)}`);
    }
    // Идэвхтэй төлбөрүүд (0-аас илүү)
    const activePayments = Object.entries(payments)
      .filter(([, v]) => Number(v) > 0)
      .map(([method, amount]) => ({ method, amount: Number(amount) }));
    if (activePayments.length === 0) return alert("Төлбөрийн төрөл сонгоно уу");
    ...
    <div className="grid h-[calc(100vh-100px)] grid-cols-1 lg:grid-cols-[420px_1fr] gap-3">
            ...
            {PAY_OPTIONS.map((p) => (
              <div key={p.value} className="flex items-center gap-2">
                ...
                <input
                  ...
                  onChange={(e) => setPayment(p.value, e.target.value)}
                  placeholder="0"
                  className="flex-1 rounded-md border border-ink/15 bg-paper px-2 py-1.5 text-right text-sm outline-none focus:border-beak"
                />
                <button
                  onClick={() => autoFillRemaining(p.value)}
                  className="text-xs px-2 py-1 rounded-md bg-cream hover:bg-beak-100 transition"
                  title="Үлдсэн дүнг автоматаар бөглөх"
                >
                  Бүх
                </button>
              </div>
      ...
      <div className="card flex flex-col overflow-hidden">
        <div className="border-b border-ink/10 p-3 space-y-2">
          ...
          <div className="flex flex-wrap gap-1.5">
            ...
            {cats.map((c) => {
              ...
              return (
                <button
                  key={c.id}
                  onClick={() => { setActiveCat(c.id); setActiveBrand("all"); }}
                  className={`rounded-full px-3 py-1.5 text-xs font-semibold transition ${
          ...
          {/* Брэндүүд (зөвхөн ангилал сонгосон үед) */}
          {brandsInCategory.length > 0 && (
            <div className="flex flex-wrap gap-1.5 pt-1 border-t border-ink/5">
              <button
                onClick={() => setActiveBrand("all")}
                className={`rounded-full px-3 py-1 text-xs transition ${
                  activeBrand === "all" ? "bg-beak text-ink font-semibold" : "bg-cream/50 text-ink-400 hover:bg-beak-100"
                }`}
              >
                🏷 Бүх брэнд
              </button>
              {brandsInCategory.map((b) => (
                <button
                  ...
                  className={`rounded-full px-3 py-1 text-xs transition ${
                    activeBrand === b ? "bg-beak text-ink font-semibold" : "bg-cream/50 text-ink-400 hover:bg-beak-100"
                  }`}
                >
                  {b}
                </button>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3">
            {filtered.map((p) => (
              ...
              >
                ...
                <div className="p-2">
                  {p.brand && <p className="text-[10px] text-beak-600 font-bold uppercase">{p.brand}</p>}
            ...
            <div className="space-y-1.5">
              {pickProduct._variants.filter((v) => v.stock > 0).map((v, i) => {
                ...
                return (
                  <button
                    key={i}
                    onClick={() => addToCart(pickProduct, v)}
