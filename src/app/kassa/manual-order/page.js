...
export default function KassaManualOrderPage() {
  ...
  async function submit() {
    ...
    const res = await fetch("/api/manual-order", {
      ...
      body: JSON.stringify({
        ...
        order_date: customer.order_date, branch: customer.branch,
        items: items.map((it) => ({
          ...
          <div className="card p-4">
            ...
            <div className="grid grid-cols-2 gap-2">
              {[
                ...
                <button key={b.value} onClick={() => setCustomer({ ...customer, branch: b.value })}
